    ...
    def clean(self):
        ...
        if cleaned_data.get('has_cd4') == YES and not cleaned_data.get('cd4_result'):
            raise forms.ValidationError(
                'If CD4 results are available, what is the value of the result?')
        ...
        if cleaned_data.get('has_cd4') == NO and cleaned_data.get('cd4_result'):
            raise forms.ValidationError(
                'NO CD4 results are available, DO NOT GIVE a CD4 result value?')
        ...
        if cleaned_data.get('nadir_cd4') > cleaned_data.get('cd4_result'):
            raise forms.ValidationError(
                'The LOWEST CD4 CANNOT be GREATER than the most RECENT CD4 result')
        if cleaned_data.get('cd4_result') < cleaned_data.get('nadir_cd4'):
            raise forms.ValidationError(
                'The most RECENT CD4 CANNOT be LESS than the LOWEST CD4 result')

        if cleaned_data.get('has_vl') == YES and not cleaned_data.get('vl_result'):
            raise forms.ValidationError(
                'If VL results are available, What is the value of the result?')
        if cleaned_data.get('vl_result') and not cleaned_data.get('vl_drawn_date'):
            raise forms.ValidationError(
                'You have provided the VL result value, when was the draw date?')
